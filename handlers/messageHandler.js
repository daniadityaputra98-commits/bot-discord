...
const genderPrefs = require("../utils/genderPrefs");

const lastInteraction = new Map(); // stateKey -> timestamp, buat cooldown sapaan

// Balasan sapaan singkat, tetap ramah namun berkelas.
const SMALL_TALK = {
  wo: ["Ya, ada yang bisa saya bantu?", "Saya di sini, silakan."],
  wok: ["Baik, ada apa gerangan?", "Silakan, saya mendengarkan."],
  wowo: ["Ya, ada apa?", "Saya di sini, silakan berkenan bicara."],
  woo: ["Ya?", "Silakan, ada apa?"],
  woy: ["Ya, saya dengar.", "Silakan, ada apa?"],
  oi: ["Ya?", "Silakan, ada apa?"],
  bot: ["Ya, ada apa?", "Dengan senang hati, ada yang bisa saya bantu?"],
  bro: ["Ya, silakan, ada apa?"],
  bang: ["Ya, silakan, ada apa?"],
  min: ["Ya, ada apa?"],
};
...
async function JonileCommand(message, cmd, args) {
  ...
  switch (cmd) {
    case "help": {
      return message.reply(
        [
          ...
          `\`${prefix}panggilan pria/wanita/netral\` — atur manual sebutan (kalau tidak punya role Kings/Cuties)`,
          "",
          "Selain perintah di atas, ucapkan kata-kata tertentu (wkwk, sedih, rizz, delulu, dst) di percakapan manapun, dan saya akan dengan senang hati mengirimkan GIF yang sesuai.",
        ].join("\n")
      );
    }
    ...
    case "join": {
      ...
      if (!voiceChannel) return message.reply("Silakan masuk ke voice channel terlebih dahulu, baru saya akan menyusul.");

      const existing = voiceKeeper.getVoiceConnection(message.guild.id);
      if (existing) {
        if (existing.joinConfig.channelId === voiceChannel.id) {
          return message.reply("Saya sudah berada di sana, Tuan/Nyonya.");
        }
        existing.destroy();
      }
      ...
      return message.reply(
        `Baik, saya akan bergabung ke **${voiceChannel.name}** dan menemani di sana. Bila ingin saya undur diri, silakan gunakan \`${prefix}leave\`.`
      );
    }

    case "leave": {
      const ok = voiceKeeper.leave(message.guild.id);
      return message.reply(ok ? "Baik, saya permisi undur diri dari voice channel." : "Maaf, sepertinya saya memang belum berada di voice channel manapun.");
    }
    ...
    case "kerang": {
      ...
      if (!question) return message.reply(`Silakan ajukan pertanyaan Anda. Contoh: \`${prefix}kerang apakah aku akan kaya?\``);
    ...
    case "prefix": {
      ...
      if (!message.member.permissions.has("ManageGuild")) {
        return message.reply("Mohon maaf, Anda memerlukan izin **Manage Server** untuk mengubah prefix.");
      }
    ...
    case "panggilan": {
      ...
      if (!sub || sub === "help") {
        return message.reply([
          "**Sebutan Panggilan**",
          "Kalau kamu punya role Kings atau Cuties di server ini, sebutannya otomatis mengikuti role tersebut.",
          `Kalau belum punya role itu, atur manual: \`${prefix}panggilan pria\` (Tuan) / \`${prefix}panggilan wanita\` (Nyonya) / \`${prefix}panggilan netral\` (nama saja).`,
        ].join("\n"));
      }

      const result = genderPrefs.set(message.author.id, sub);
      if (!result.ok) return message.reply(`❌ ${result.error}`);

      if (!result.value) return message.reply("Baik, mulai sekarang saya akan memanggil Anda dengan nama saja.");
      const label = result.value === "pria" ? "Tuan" : "Nyonya";
      return message.reply(`Dengan senang hati, mulai sekarang saya akan memanggil Anda "${label}".`);
    }
    ...
    case "custom": {
      ...
      if (!message.member.permissions.has("ManageGuild")) {
        return message.reply("Mohon maaf, Anda memerlukan izin **Manage Server** untuk mengatur custom command.");
      }
...
module.exports = async (message) => {
  ...
  return message.reply(textHelper.random([
    "Mohon maaf, saya kurang menangkap maksud Anda. Bolehkah diulangi?",
    "Seribu maaf, sepertinya saya perlu mendengarnya sekali lagi.",
  ]));
};
